...
The shelf is split into two category sections, in this order: **Fiction**, then **Non-Fiction**. The grid itself uses `grid-template-columns:repeat(auto-fill, minmax(260px, 1fr))`, so it fills more columns on wide screens instead of staying a fixed 2-up list as more books are added. A sticky "Fiction / Non-Fiction" jump nav sits under the hero and highlights whichever section is currently in view (`assets/shelf.js`).

### Progressive reveal ("infinite scroll" without a backend)

Every book tile is still written directly into `index.html`, in DOM order, exactly as before — this keeps the site fully readable with JS off and keeps every book indexable by search engines. `assets/shelf.js` decides at runtime how much to *show*: each category grid only displays its first 6 tiles at first, then reveals the next 6 as the reader scrolls near the bottom (via `IntersectionObserver`), with a short fade-in. The "next booktalk coming soon" placeholder tile is held back and only reappears once every real tile in that category has been revealed. **Nothing about adding a new booktalk changes** — keep appending one `.shelf-tile` to the matching Fiction/Non-Fiction grid; the script reads however many tiles exist and reveals them in batches automatically once a category passes 6 entries. If JS fails to load, everything just displays normally (no hidden content). Each section is its own `.shelf-section` block with a heading, a live count, and its own `.shelf-grid` of tiles — plus a dashed "next booktalk coming soon" placeholder tile at the end of *that* category's grid (not one shared placeholder for the whole site). The overall `.shelf-count` line at the top of the page shows the total plus the fiction/non-fiction split, e.g. "3 booktalks · 1 fiction · 2 non-fiction." When a new booktalk is added, its tile goes into the matching category section, and both the section count and the top-level count get updated.
